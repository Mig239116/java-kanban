...
class HttpTaskManagerTasksTest {
    ...
    public void shouldCreateTask() throws IOException, InterruptedException {
        ...
        int newTaskID = jsonElement.getAsJsonObject().get("taskID").getAsInt();
        String message =jsonElement.getAsJsonObject().get("message").getAsString();
        assertEquals("Задача создана", message);
        List<Task> tasksFromManager = manager.getAllTasks();
        assertNotNull(tasksFromManager, "Задачи не возвращаются");
        assertEquals(1, tasksFromManager.size(), "Некорректное количество задач");
        Task task = tasksFromManager.getFirst();
        assertEquals(newTaskID, task.getTaskID(), "Номер задачи не существует");
    ...
    public void shouldCreateSubtask() throws IOException, InterruptedException {
        ...
        JsonElement jsonElement = JsonParser.parseString(response.body());
        assertTrue(jsonElement.isJsonObject());
        int newSubtaskID = jsonElement.getAsJsonObject().get("taskID").getAsInt();
        String message =jsonElement.getAsJsonObject().get("message").getAsString();
        assertEquals("Подзадача создана", message);
        List<Subtask> tasksFromManager = manager.getAllSubtasks();
        assertNotNull(tasksFromManager, "Задачи не возвращаются");
        assertEquals(1, tasksFromManager.size(), "Некорректное количество задач");
        Subtask subtask = tasksFromManager.getFirst();
        assertEquals(newSubtaskID, subtask.getTaskID(), "Номер задачи не существует");
    ...
    public void shouldCreateEpic() throws IOException, InterruptedException{
        ...
        JsonElement jsonElement = JsonParser.parseString(response.body());
        assertTrue(jsonElement.isJsonObject());
        int newEpicID = jsonElement.getAsJsonObject().get("taskID").getAsInt();
        String message =jsonElement.getAsJsonObject().get("message").getAsString();
        assertEquals("Эпик создан", message);
        List<Epic> tasksFromManager = manager.getAllEpics();
        assertNotNull(tasksFromManager, "Задачи не возвращаются");
        assertEquals(1, tasksFromManager.size(), "Некорректное количество задач");
        Epic epic = tasksFromManager.getFirst();
        assertEquals(newEpicID, epic.getTaskID(), "Номер задачи не существует");
